"""Generate blowfish_constants.py from the standard hex digits of pi."""
...
with open('crypto/blowfish_constants.py', 'w') as f:
    f.write('"""Blowfish P-array and S-box constants (from hex digits of pi)."""\n\n')
    
    def write_array(name, arr, per_line=8):
        f.write(f'{name} = [\n')
    ...
    write_array('P_ORIG', p)
    write_array('S0_ORIG', s0)
    write_array('S1_ORIG', s1)
    write_array('S2_ORIG', s2)
    write_array('S3_ORIG', s3)

print(f"Generated crypto/blowfish_constants.py")
